...
template <typename VecS, typename MatU, typename MatV>
struct svd_states
{
    svd_states(VecS& sigma, MatU& U, MatV& V) : sigma_(sigma), U_(U), V_(V)
    {
    }
    arma::uword i_;
    VecS& sigma_;
...
void one_sided_jacobi_svd(MatA& A, VecS& sigma, MatV& V, RealT tol)
{
    using size_type  = arma::uword;
    using value_type = typename MatA::elem_type;
    using real_type  = typename MatA::pod_type;
    using jacobi_aux = detail::one_sided_jacobi_helper<value_type>;
...
#ifdef DEBUG
        ++count;
        std::cout << "(one_sided_jacobi_svd) iter " << std::setw(8) << count
                  << ": max residual = " << max_resid << '\n';
#endif /* DEBUG */
    } while (max_resid > tol);
    //
